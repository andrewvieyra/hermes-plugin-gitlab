...
* Events are queued and sent from one daemon thread. Emitting never blocks a GitLab operation.
...
Standard library only. Configuration lives in ``plugins.entries.gitlab.settings.audit_sinks``::
...
_WARNING_EVENTS = {"write_refused", "write_rejected", "write_failed", "write_conflict", "read_failed", "staged_expired"}
_NOTICE_EVENTS = {"write_done", "write_staged", "staged_run", "staged_dropped", "staged_pruned"}


def severity_of(record: Dict[str, Any]) -> int:
    """Syslog severity for an audit record: warning for anything that failed or was refused."""
    event = str(record.get("event", ""))
    if event in _WARNING_EVENTS:
        return SEV_WARNING
    if event == "staged_run" and record.get("outcome") != "done":
        return SEV_WARNING
...
def to_cef(record: Dict[str, Any]) -> str:
    ...
    ext += [
        ...
        ("cs6Label", "project"),
        ("cs6", record.get("project")),
    ]
    if record.get("action"):
        ext.append(("act", record["action"]))
    if target.get("iid") is not None or target.get("id") is not None:
        ext.append(("externalId", target.get("iid") if target.get("iid") is not None else target.get("id")))
    if target.get("web_url"):
        ext.append(("cs7Label", "web_url"))
        ext.append(("cs7", target["web_url"]))
    if record.get("http_status") is not None:
        ext += [("cn1Label", "http_status"), ("cn1", record["http_status"])]
    if record.get("outcome"):
        ext.append(("outcome", record["outcome"]))
    msg = record.get("error") or record.get("reason") or record.get("summary")
    if msg:
        ext.append(("msg", msg))
    if actor.get("request"):
        ext += [("cs8Label", "request_text"), ("cs8", actor["request"])]
...
def build_sink(spec: Dict[str, Any]) -> Optional[Sink]:
    """One sink from its config mapping; ``None`` (with a warning) for an unusable spec."""
    if not isinstance(spec, dict):
        logger.warning("gitlab audit_sinks: entry is not a mapping: %r", spec)
        return None
    ...
    try:
        if kind == "syslog":
            ...
            return SyslogSink(
                ...
                app_name=str(spec.get("app_name", "hermes-gitlab")),
        ...
        if kind == "http":
            return HttpSink(
                ...
                timeout=float(spec.get("timeout", 5.0)),
                verify_ssl=bool(spec.get("verify_ssl", True)),
                sourcetype=str(spec.get("sourcetype", "hermes:gitlab")),
            )
        raise ValueError(f"unknown sink type {kind!r} (expected syslog or http)")
    except Exception as exc:
        logger.warning("gitlab audit_sinks: ignoring entry %r: %s", spec, exc)
        return None
...
class SinkWorker:
    ...
    def __init__(
        ...
        self._thread = threading.Thread(target=self._run, name="gitlab-audit-sinks", daemon=True)
        if self.sinks:
            self._thread.start()
    ...
    def _log_once(self, key: str, message: str) -> None:
        ...
        if now - self._last_error_log.get(key, -ERROR_LOG_INTERVAL) >= ERROR_LOG_INTERVAL:
            ...
            logger.warning("gitlab %s", message)
...
_worker: Optional[SinkWorker] = None
...
def set_worker(worker: Optional[SinkWorker]) -> None:
...
def dispatch(record: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.debug("gitlab audit sink dispatch failed: %s", exc)
