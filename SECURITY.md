...
| Secrets in CI logs, repository files or user-written text reach the model provider | `redact_secrets` masks token formats, private keys, bearer headers and basic-auth URLs in job logs, file contents, diffs, code-search snippets, issue and MR descriptions, comments and commit messages, plus `password=`-style assignments in job logs; raw `gitlab_api` results (GET and write responses) are redacted like every other read. It is pattern-based and not exhaustive. |
...
- Its secret redaction is pattern-based: a secret in an unusual format passes through, in a raw
  `gitlab_api` response as much as anywhere else.
