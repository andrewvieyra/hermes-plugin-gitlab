...
| Secrets in CI logs, repository files or user-written text reach the model provider | `redact_secrets` masks token formats, private keys, bearer headers and basic-auth URLs in job logs, file contents, diffs, code-search snippets, issue and MR descriptions, comments and commit messages, plus `password=`-style assignments in job logs; raw `gitlab_api` results (GET and write responses) are scanned for token formats too. It is pattern-based and not exhaustive. |
